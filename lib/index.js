...
function Task(title, body, importance, id, completed, number) {
  this.title = title;
  this.body = body;
  ...
  return (`
    <li class="task-card" id=${this.id}>
      <header class="bottom-header">
        ...
        <button class="complete-btn complete">Complete</button>
        <h3 contenteditable="true" class=${this.completed}>${this.title}</h3>
      </header>
      <p contenteditable="true" class=${this.completed}"card-body">${this.body}</p>
      <footer>
        <button class="upvote-btn upvote">Upvote</button>
        <button class="downvote-btn downvote">Downvote</button>
        <h4>importance:<span class="importance-change"> ${this.importance}</span></h4>
      </footer>
    </li>
    `);
  };
...
//limit this function to accept a number argument
  renderOnlyTen: function () {
    // console.log('length', this.taskArray.length);
    // console.log('title', this.taskArray[0].title);
    taskSection.html('');
  ...
  // retrieveOnlyTen: function() {
  //   var retrievedTasks = JSON.parse(localStorage.getItem('tasks'));
  //   if (retrievedTasks) {
  //     for (i = retrievedTasks.length-10; i < retrievedTasks.length; i++) {
