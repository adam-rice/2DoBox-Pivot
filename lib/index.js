...
Task.prototype.toHTML = function() {
  return (`
    <li class="task-card" id=${this.id}>
      ...
      <p contenteditable="true" class=${this.completed} id="card-body">${this.body}</p>
      <footer>
...
taskSection.on('click', '.destroy-btn, .upvote-btn, .downvote-btn, .complete-btn', function() {
  ...
  if (this.id === 'upvote') {
    ...
    // $(this).closest('h3').attr('style', 'text-decoration:line-through');
    console.log(find.completed);
  } else find.remove();
});
...
taskManager.render();
var lessThanTen = taskManager.taskArray.filter(showOnlyTen);
