...
var loadMoreButton = $('#load-more');
  ...
  Task.prototype.toHTML = function() {
    ...
    return (`
      <li class="task-card ${this.completed}" id=${this.id}>
        ...
        <footer>
          <button class="upvote-btn upvote"></button><button class="downvote-btn downvote"></button>
          <h4>
  ...
  var taskManager = {
    ...
    renderOnlyTen: function () {
      taskSection.html('');
      var task = this.taskArray.concat().reverse().slice(0, 10);
      console.log(task)
      //task.toHTML is not a function//
      taskSection.prepend(task.toHTML());
    },
